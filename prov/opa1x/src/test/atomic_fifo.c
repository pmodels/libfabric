...
static inline void barrier_enter (struct barrier_participant * participant) {
	...
	fi_opa1x_compiler_inc_u64(participant->started[index]);
}
...
static inline void barrier_wait (struct barrier_participant * participant) {
	...
	const uint64_t result = fi_opa1x_compiler_fetch_and_inc_u64(completed);
	if (result == (count-1)) {
		*started = 0;
		*completed = 0;
	}
...
struct producer_test {
	struct fi_opa1x_atomic_fifo_producer	producer;
	double					elapsed_usec;
};
...
void test_producer (struct producer_info * info, unsigned index, const unsigned do_produce_unsafe) {
	...
	struct fi_opa1x_atomic_fifo_producer * producer = &info->test[index].producer;
	...
	union fi_opa1x_timer_state timer;
	union fi_opa1x_timer_stamp start;
	fi_opa1x_timer_init(&timer);
	...
	if (do_produce_unsafe) {
		...
		for (i=1; i<=exclude_iterations; ++i) {
			...
			fi_opa1x_atomic_fifo_produce_unsafe(producer, data_lsh3b);
		}
		...
		for (i=1; i<=exclude_iterations; ++i) {
			const uint64_t data_lsh3b = id | (i<<3);
			fi_opa1x_atomic_fifo_produce(producer, data_lsh3b);
		}
	...
	info->test[index].elapsed_usec =
		fi_opa1x_timer_elapsed_usec(&start, &timer);
...
void test_consumer (struct consumer_info * info, const unsigned index, const unsigned do_consume_wait, const unsigned do_validation) {
	...
	struct barrier_participant * barrier = &info->barrier;
	struct fi_opa1x_atomic_fifo * fifo = &info->test[index].fifo;
	...
	for (i=0; i<exclude_iterations; ++i) {
		...
		if (do_consume_wait) {
			fi_opa1x_atomic_fifo_consume_wait(fifo, &data_lsh3b);
		} else {
			int rc = 1;
			do {
				rc = fi_opa1x_atomic_fifo_consume(fifo, &data_lsh3b);
			} while (rc != 0);
		}
	...
	const double elapsed_usec = fi_opa1x_timer_elapsed_usec(&start, &timer);
...
int main (int argc, char * argv[]) {
	...
	fi_opa1x_atomic_fifo_init(&c.test[0].fifo, 16);
	fi_opa1x_atomic_fifo_init(&c.test[1].fifo, 256);
	fi_opa1x_atomic_fifo_init(&c.test[2].fifo, 1024);
	...
	for (j=0; j<num_producers; ++j) {
		...
		p[j].test_count = 3;
		for (i=0; i<3; ++i) {
			fi_opa1x_atomic_fifo_producer_init(&p[j].test[i].producer, &c.test[i].fifo);
		}
